...
export const onRequestPost = async (context: Parameters<PagesFunction>[0]) => {
  ...
  try {
    ...
    await db.prepare("UPDATE agent_tasks SET status = '" + "'cancelled'" + "', finished_at = datetime('now') WHERE id = ?").run(taskId);
    return jsonResponse({ success: true, taskId, status: "cancelled" }, 200);
  } catch (e) {
    return jsonResponse({ code: "CANCEL_ERROR", message: String(e) }, 500);
  }
};
