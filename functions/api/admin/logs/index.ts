...
export const onRequestGet = async (context: Parameters<PagesFunction>[0]) => {
    ...
    try {
        ...
        if (requestId) { query += " AND request_id=?"; params.push(requestId); }
        query += " ORDER BY created_at DESC LIMIT ? OFFSET ?";
        params.push(limit, offset);
        const rows: any[] = await db.prepare(query).bind(...params).all();
        const total: any = await db.prepare(
            "SELECT COUNT(*) as cnt FROM system_logs WHERE 1=1" +
            (level ? " AND level=?" : "") +
            (module ? " AND module=?" : "") +
            (requestId ? " AND request_id=?" : "")
        ).bind(...params.slice(0, 3)).first();
        return jsonResponse({
            logs: (rows||[]).map((r: any) => ({ id:r.id, requestId:r.request_id, level:r.level, module:r.module, message:r.message, metadata: r.metadata ? JSON.parse(r.metadata) : {}, createdAt: r.created_at })),
