...
export function validateProfileUpdate(input: unknown): ProfileUpdateInput {
  ...
  const allowedFields = new Set(["displayName", "avatarUrl", "locale", "timezone"]);
  ...
  return { displayName, avatarUrl, locale, timezone };
}
  ...
  await db
    .prepare(
      `INSERT INTO profiles
        (user_id, display_name, avatar_url, locale, timezone, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)
